# High-Win-Rate Trading Simulation
# Strategy: Only take trades when ALL high-probability conditions align

$marketDataPath = "C:\Users\sixer\.config\kilo\output\trading\simulation\market_data_90d.json"
$outputPath = "C:\Users\sixer\.config\kilo\output\trading\simulation\improved_results.json"

Write-Host "Loading market data..." -ForegroundColor Cyan
$json = Get-Content $marketDataPath -Raw | ConvertFrom-Json
$coins = $json.coins

# Configuration - HIGHLY SELECTIVE
$initialCapital = 10000
$maxRiskPerTrade = 0.02

# Strict thresholds for 70%+ win rate
$minRSIBuy = 30      # Only extreme oversold (< 30)
$minRSISell = 70     # Only extreme overbought (> 70)
$minVolRatio = 1.5   # Strong volume spike
$minMomentum = 0     # Price must be rising (for BUY)
$requireUptrend = $true  # EMA50 must be above EMA200 for BUY
...
Write-Host "`n=== HIGH-PROBABILITY SELECTIVE TRADING ===" -ForegroundColor Magenta
Write-Host "Strategy: Only trade when ALL conditions align" -ForegroundColor Gray
Write-Host "RSI < $minRSIBuy for BUY | RSI > $minRSISell for SELL | Vol > $minVolRatio | Momentum >= $minMomentum" -ForegroundColor Gray
...
foreach ($coinName in $coins.PSObject.Properties.Name) {
    ...
    for ($i = 25; $i -lt $prices.Count - 7; $i++) {
        ...
        # Bollinger lower band position
        $sma20Sum = 0
        for ($j = $i - 19; $j -le $i; $j++) { $sma20Sum += $prices[$j] }
        $sma20 = $sma20Sum / 20
        $sumSq = 0
        for ($j = $i - 19; $j -le $i; $j++) { $sumSq += [Math]::Pow($prices[$j] - $sma20, 2) }
        $std = [Math]::Sqrt($sumSq / 20)
        $bbLower = $sma20 - (2 * $std)
        $bbPos = if ($std -gt 0) { ($prices[$i] - $bbLower) / ($std * 4) } else { 0.5 }

        # High-probability BUY signal
        $buySignal = $false
        if ($rsi -ne $null -and $rsi -lt $minRSIBuy -and
            $volRatio -ge $minVolRatio -and
            $momentum3 -ge $minMomentum -and
            ($requireUptrend -eq $false -or $uptrend)) {
            $buySignal = $true
        }

        # High-probability SELL signal
        $sellSignal = $false
        if ($rsi -ne $null -and $rsi -gt $minRSISell -and
            $volRatio -ge $minVolRatio -and
            $momentum3 -le (0 - $minMomentum) -and
            ($requireUptrend -eq $false -or $uptrend -eq $false)) {
            $sellSignal = $true
        }
        ...
        if ($buySignal -or $sellSignal) {
            ...
            $trades += [PSCustomObject]@{
                ...
                momentum = [Math]::Round($momentum3, 2)
            }
...
Write-Host "`nTotal signals generated: $signalsGenerated" -ForegroundColor Yellow
Write-Host "Trades executed: $($trades.Count)" -ForegroundColor Yellow
...
Write-Host "   HIGH-PROBABILITY TRADING RESULTS" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "Strategy: RSI<$minRSIBuy/>$minRSISell + Vol>$minVolRatio + Momentum>=$minMomentum"
...
Write-Host "Win Rate: $($result.win_rate_percent)% ($($result.winning_trades) wins / $($result.losing_trades) losses)" -ForegroundColor $(if ($winRate -ge 0.7) { "Green" } else { "Yellow" })
...
if ($winRate -ge 0.7) {
    ...
    Write-Host "Win rate below 70% target - needs adjustment" -ForegroundColor Yellow
}
Write-Host ""
